# Recovery Record Contract v1.8.0

The canonical recovery record continues to contain `metadata`, `user_input`, `normalized_input`, `findings`, `human_review`, and `extensions`. The v1.8.0 evidence, assumption, and handoff ledgers are private workspace entities linked to the canonical record and revision IDs rather than silently embedded into or rewriting historical records.
