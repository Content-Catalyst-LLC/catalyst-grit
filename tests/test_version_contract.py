...
def test_all_release_surfaces_match_version_file():
    ...
    assert version == "1.9.0" == __version__ == ENGINE_VERSION == SCHEMA_VERSION
    ...
    for name in ["catalyst_grit_record.schema.json", "catalyst_grit_request.schema.json", "catalyst_grit_methodology_profile.schema.json", "catalyst_grit_project.schema.json", "catalyst_grit_workspace_bundle.schema.json", "catalyst_grit_publication.schema.json", "catalyst_grit_api_response.schema.json"]:
        schema = json.loads((ROOT / "schemas" / name).read_text())
        assert schema["x-catalyst-grit-version"] == version
    assert re.search(r'^version = "1\.9\.0"$', (ROOT / "pyproject.toml").read_text(), re.M)
    plugin = (ROOT / "wordpress/catalyst-grit-demo/catalyst-grit-demo.php").read_text()
    assert "Version: 1.9.0" in plugin and "CATALYST_GRIT_DEMO_VERSION', '1.9.0'" in plugin
    assert re.search(r"^  version: 1\.9\.0$", (ROOT / "openapi.yaml").read_text(), re.M)


def test_installer_uses_isolated_local_source_smoke_test():
    installer = (ROOT / "install_and_push_catalyst_grit_v1_9_0_macos.sh").read_text()
    assert 'INSTALLER_REVISION="CHECKSUM_SYNC_STATE_SAFE_V1"' in installer
