#!/usr/bin/env python3
"""Build deterministic Catalyst Grit v1.2.0 release artifacts."""
...
def included_files(base: Path):
    for path in sorted(base.rglob("*"), key=lambda item: item.as_posix()):
        ...
        if any(part in EXCLUDED_PARTS or part.endswith(".egg-info") for part in relative.parts): continue
        if path.suffix in EXCLUDED_SUFFIXES: continue
        if relative.parts and relative.parts[0] == "outputs" and path.name != ".gitkeep": continue
        yield path, relative
...
def main() -> int:
    ...
    source_zip=DIST/f"catalyst-grit-v{VERSION}-repository-{REPAIR_TAG}.zip"
    ...
    installer_target=DIST/f"install_and_push_catalyst_grit_v{VERSION.replace('.', '_')}_{REPAIR_TAG}_macos.sh"; shutil.copy2(installer_source, installer_target)
    artifacts=[]
    for path in (source_zip, plugin_zip, installer_target): artifacts.append({"file":path.name,"bytes":path.stat().st_size,"sha256":digest(path)})
    manifest={"product":"Catalyst Grit","version":VERSION,"release":"Persistent Records, Projects, and Review Checkpoints","installer_revision":INSTALLER_REVISION,"repair":"checksum-based synchronization plus isolated local-source smoke validation","generated_at":datetime.now(timezone.utc).replace(microsecond=0).isoformat(),"artifacts":artifacts}
    ...
    bundle=DIST/f"catalyst-grit-v{VERSION}-release-{REPAIR_TAG}.zip"
    with zipfile.ZipFile(bundle,"w",compression=zipfile.ZIP_DEFLATED,compresslevel=9) as archive:
        for path in sorted(DIST.iterdir(),key=lambda item:item.name):
            if path != bundle and path.is_file(): add_file(archive,path,path.name)
