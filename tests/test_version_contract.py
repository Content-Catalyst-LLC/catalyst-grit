...
def test_all_release_surfaces_match_version_file():
    ...
    assert version == "1.4.0" == __version__ == ENGINE_VERSION == SCHEMA_VERSION
    ...
    assert re.search(r'^version = "1\.4\.0"$', (ROOT / "pyproject.toml").read_text(), re.M)
    plugin = (ROOT / "wordpress/catalyst-grit-demo/catalyst-grit-demo.php").read_text()
    assert "Version: 1.4.0" in plugin and "CATALYST_GRIT_DEMO_VERSION', '1.4.0'" in plugin
    assert re.search(r"^  version: 1\.4\.0$", (ROOT / "openapi.yaml").read_text(), re.M)


def test_installer_uses_isolated_local_source_smoke_test():
    installer = (ROOT / "install_and_push_catalyst_grit_v1_4_0_macos.sh").read_text()
