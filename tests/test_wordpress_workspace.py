...
def test_private_workspace_requires_authentication_capability_and_nonce():
    ...
    assert "check_ajax_referer('catalyst_grit_workspace_v180', 'nonce')" in PHP
...
def test_team_workspace_exposes_consent_roles_and_legacy_workspace_migration():
    assert "Private Evidence and Recovery Workspace" in PHP
    ...
    assert "evidence_items" in PHP and "assumptions" in PHP and "handoffs" in PHP and "monitoring_snapshots" in PHP and "monitoring_reviews" in PHP
    assert "evidence_items" in PRIVATE_JS and "assumptions" in PRIVATE_JS and "handoffs" in PRIVATE_JS and "monitoring_snapshots" in PRIVATE_JS and "monitoring_reviews" in PRIVATE_JS
