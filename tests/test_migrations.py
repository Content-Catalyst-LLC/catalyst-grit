...
def test_clean_install_rollback_and_remigration(tmp_path: Path):
    ...
    with SQLiteWorkspaceRepository(database, auto_migrate=False) as repo:
        assert repo.migrations.status() == {"current": 0, "latest": 2, "applied": [], "pending": [1, 2]}
        assert repo.migrations.migrate() == [1, 2]
        assert repo.migrations.status()["current"] == 2
        assert repo.migrations.rollback(1) == [2]
        assert repo.migrations.status()["current"] == 1
        assert repo.migrations.migrate() == [2]
