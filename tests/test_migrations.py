...
def test_packaged_migrations_are_ordered_and_complete():
    ...
    assert [item.name for item in migrations] == ["core_workspace", "checkpoints_reviews_audit", "recovery_plans_actions_reassessment", "learning_loops_adaptation_patterns"]
...
def test_clean_install_rollback_and_remigration(tmp_path: Path):
    ...
    with SQLiteWorkspaceRepository(database, auto_migrate=False) as repo:
        assert repo.migrations.status() == {"current": 0, "latest": 4, "applied": [], "pending": [1, 2, 3, 4]}
        assert repo.migrations.migrate() == [1, 2, 3, 4]
        assert repo.migrations.status()["current"] == 4
        assert repo.migrations.rollback(1) == [4]
        assert repo.migrations.status()["current"] == 3
        assert repo.migrations.migrate() == [4]
